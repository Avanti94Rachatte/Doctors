...
const DoctorProfile = () => {
  ...
  const {dToken,backendUrl, getProfileData, profileData, setProfileData,} = useContext(DoctorContext)
  ...
  return profileData && (
    ...
    <div className=''>
      <div className="flex flex-col gep-4 m-5">
        <div className="">
       ...
       {/*   Doctor info : name , degree, experience */}
        ...
        <p className="flex items-center font-medium gap-2 text-gray-700 text-3xl">{profileData.name}</p>
      ...
      <div className="flex gap-1 pt-2">
        <input onChange={()=>isEdit && setProfileData(prev => ({...prev, available: !prev.available}))} checked={profileData.available} type="checkbox" className="" />
        <label htmlFor="" className="">Available</label>
      </div>
      {
        isEdit
        ?<button onClick={udateProfile} className="px-4 py-1 border border-primary text-sm rounded-full mt-5 hover:bg-primary hover:text-white transition-all">SAVE</button>
