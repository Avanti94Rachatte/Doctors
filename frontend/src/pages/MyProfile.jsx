      ...
      
      {
        ...
        ? <label htmlFor="image" className="">
           <div className="inline-block relative cursor-pointer">
            <img src={image? URL.createObjectURL(image):(userData.image || assets.profile_pic)} alt="" className="w-36 rounded opacity-75" />
            <img src={!image && (
  <img src={assets.upload_icon} alt="" className="w-10 absolute bottom-12 right-12" />
)} alt="" className="w-10 absolute bottom-12 right-12" />
           </div>
      ...
      <div className="mt-10">
        {
          isEdit 
          ? <button onClick={updateUserProfileData} className="border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all duration-500">Save Infornation</button>
          : <button onClick={()=>setIsEdit(true)} className="border border-primary px-8 py-2 rounded-full  hover:bg-primary hover:text-white transition-all duration-500">Edit</button>
        }
      </div>
    </div>
  )
